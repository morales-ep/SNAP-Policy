...
- SNAP Policy Database Data Dictionary URL: [SNAP Policy Database](https://www.ers.usda.gov/data-products/snap-policy-data-sets/documentation/)
  - **Broad-Based Cateogrical Eligibility** [info](https://fns-prod.azureedge.net/sites/default/files/resource-files/BBCEStatesChart%28May2020%29.pdf)

# CONTINUE HERE
- Clean this Markdown file/code, finalize code for R Shiny script file, push to Github repo, and then edit Github repo **README** to be conducive to *reproducible research*
  - Refer to [Dialysis Analysis](https://github.com/jessicamizzi/Prop23-CA-DataChallenge-2020)

## Research Questions
1. How many States had robust SNAP policies at the end or in the final year of the data available?

2. How did overall SNAP outreach spending change throughout the database's timeframe?

3. How did certification period change over time or after a particular year? Does certification period change when including relevant policies?

4. What was the frequency of SNAP policies that address food insecurity and social determinants of health overall?

**Variables of Interest:** *outreach, transben, cap, bbce, bbce_inclmt, bbce_asset, bbce_a_amt, bbce_hh, bbce_sen, call, cap, certearnmed, certeldmed, certnonearnmed, faceini, facerec, oapp, outreach, reportsimple, transben*
...
snap <- read.csv(file = "https://github.com/morales-ep/Datasets/blob/master/SNAP_Policy_Database.csv?raw=true", stringsAsFactors = FALSE)
# ^ THIS METHOD WORKED!

# Lindsay's code works for some reason, not sure why
# For now, download SNAP data from Lindsay's Github repo
...
# See number of SNAP policies in 2016 in each State that use broad-based categorical eligibility to increase or eliminate the asset test and/or to increase the gross income limit for virtually all SNAP applicants
snap %>% 
...
# See number of SNAP policies in 2016 in each State that eliminates the asset test under broad-based categorical eligibility
snap %>% 
  filter(year(yearmonth) == 2016) %>%
  group_by(state_pc) %>%
  count(bbce_asset)

# States w/ SNAP policies that use broad-based categorical eligibility to increase or eliminate asset test/limit
snap %>% 
  filter(year(yearmonth) == 2016 & bbce == 1) %>%
  group_by(state_pc, bbce_asset) %>%
  count()

# Check bbce_hh status of all SNAP policies in each state in 2016
snap %>%
  filter(year(yearmonth) == 2016) %>%
  group_by(state_pc, bbce_hh) %>%
  count()

# Check SNAP policies in each State w/ no broad-based categorical eligibility or have bbce w/ limits on certain households
snap %>%
  filter(year(yearmonth) == 2016 & bbce_hh != 0) %>%
  group_by(state_pc, bbce_hh) %>%
  count()


# Check gross income limit as a percentage of the Federal poverty guidelines used under broad-based categorical eligibility by State in 2016
snap %>%
  filter(year(yearmonth) == 2016) %>%
  group_by(state_pc, bbce_inclmt) %>%
  count()
# n = 12 means the SNAP policy was implemented / stayed the same for the entire year

# Distinct gross income limit for SNAP eligibility and arrange descending order by income limit
snap %>%
  filter(year(yearmonth) == 2016 & bbce_inclmt != -9) %>%
  select(state_pc, bbce_inclmt) %>%
  distinct() %>%
  arrange(desc(bbce_inclmt))
# ^ Create bar chart (plotly or ggplot) depicting bbce income limit


# States without broad-based categorical eligibility for Gross income limit
snap %>%
  filter(year(yearmonth) == 2016 & bbce_inclmt == -9) %>%
  select(state_pc, bbce_inclmt) %>%
  distinct()
...
# bbce policy variables in 2016 by each State
snap %>%
  filter(year(yearmonth) == 2016) %>%
  ...
  arrange(desc(bbce_inclmt))
# ^ KEEP for a tab box ----
# 2 separate tab panels for States using bbce and not using bbce
...
snap %>%
  ...
  ungroup()
# ^ Include in Tab box or as a Horizontal column chart



# Count Transitional Benefits policy status for each State in 2016
snap %>%
  filter(year(yearmonth) == 2016) %>%
  group_by(state_pc) %>%
  count(transben, sort = TRUE) %>%
  distinct() %>%
  arrange(transben)
# Missing values occur in Nov-Dec 2016 (similar to outreach data, kinda)
# ^ Keep this but separate by transben status for tab box ----


# Check "call" policy variable status among all States in 2016
snap %>%
  filter(year(yearmonth) == 2016) %>%
  group_by(state_pc) %>%
  count(call, sort = TRUE)

# Calculate number of States by "call" policy status at end of 2016 fiscal year (i.e. Sept 2016)
snap %>%
  filter(year(yearmonth) == 2016 & month(yearmonth) == 9) %>%
  group_by(call) %>%
  count()

# Count number of SNAP policies in 2016 by "cap" variable status at end of 2016 FY
snap %>%
  filter(year(yearmonth) == 2016 & month(yearmonth) == 9) %>%
  group_by(cap) %>%
  count()

# Count number of SNAP policies in 2016 by "faceini" variable status at end of 2016 FY
# Waiver to use a telephone interview in lieu of a face-to-face interview at initial certification, without having to document household hardship
snap %>%
  filter(year(yearmonth) == 2016 & month(yearmonth) == 9) %>%
  group_by(faceini) %>%
  count()

# Count number of SNAP policies in 2016 by "facerec" variable status at end of 2016 FY
# Waiver to use a telephone interview in lieu of a face-to-face interview at RECERTIFICATION, without having to document household hardship
snap %>%
  filter(year(yearmonth) == 2016 & month(yearmonth) == 9) %>%
  group_by(facerec) %>%
  count()
...
# Convert to Plotly object using "ggplotly()"?
ggplotly(ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
       mapping = aes(x = faceini, y = facerec)) +
  geom_count() +
  theme_bw() +
  labs(title = "Number of States granted waivers to use phone interviews for SNAP certification, End of FY 2016",
...
ggplotly(ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
  ...
  geom_count() +
  theme_bw() +
  labs(title = "Number of States by transitional SNAP benefits and simplified reporting policies, End of FY 2016",
...
# Create frequency polygon plots for mean or mean certification period variables and noncitizen variables (i.e. "noncit...")
# Create frequency lines for each "noncit..." variables and maybe facet by "cap" variable for elderly policy variables (SSI applies to 65+ y/o)

# Frequency polygon(s) of average certification period (in months) for SNAP units w/ earnings
# Separated by "bbce_hh": State limits BBCE to certain types of households
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
       mapping = aes(x = certearnavg, color = bbce_hh)) +
  geom_freqpoly(bins = 20) +
  theme_bw()

# Frequency polygon(s) of median certification period (in months) for SNAP units w/ earnings
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
       mapping = aes(x = certearnmed, color = bbce_hh)) +
  geom_freqpoly(bins = 20) +
  theme_bw()


# Frequency polygon(s) of median certification period (in months) for SNAP units w/ earnings by "reportsimple" (simplified reporting for households w/ earnings)
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
       mapping = aes(x = certearnavg, color = reportsimple)) +
  geom_freqpoly(bins = 20) +
  theme_bw()

# Frequency polygon(s) of median certification period (in months) for SNAP units w/ earnings by "reportsimple"
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
       mapping = aes(x = certearnmed, color = reportsimple)) +
  geom_freqpoly(bins = 20) +
  theme_bw()
# States w/ simplified reporting for earning SNAP units tend to have higher certification periods
# Interesting finding that seems counterintuitive
...
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
  ...
  #facet_wrap(facets = reportsimple~., nrow = 2) +
  #scale_color_brewer(palette = "Purples") +
  theme_bw() +
  labs(title = "Median Certification Period for SNAP units with earnings by Simplified Reporting option, End of FY 2016",
       x = "Months", y = "Count")
# This plot may need to be changed for simplicity and clarity -> Remove "oapp"
# ^ Keep this freq-poly graph for certearnmed ----



# Freq-poly of avg cert period for elderly SNAP units by bbce_sen
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
       mapping = aes(x = certeldavg, color = bbce_sen)) +
  geom_freqpoly(bins = 20) +
  theme_bw()

# Frequency polygon(s) of median cert period for elderly SNAP units by bbce_sen
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
       mapping = aes(x = certeldmed, color = bbce_sen)) +
  geom_freqpoly(bins = 20) +
  theme_bw()

# Freq-poly of avg cert period for elderly SNAP units by cap
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
       mapping = aes(x = certeldavg, color = cap)) +
  geom_freqpoly(bins = 20) +
  theme_bw()
...
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
  ...
  theme_bw() +
  labs(title = "Median Certification Period for Elderly SNAP units by Combined Application Project usage, End of FY 2016",
       subtitle = "States operating a Combined Application Project streamline the SNAP application process for SSI recipients",
       x = "Months", 
       y = "Count")
# ^ Keep this freq-poly plot for certeldmed ----
# Better to use median certification period bc data seems to be skewed and mean is greatly influenced by outliers


# Create a freq-poly plot for "certnonearnmed" variable
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
  ...
  labs(title = "Median Certification Period for nonearning, nonelderly SNAP units, End of FY 2016",
       x = "Months",
       y = "Count")
# ^ Keep this freq-poly plot for noncertearnmed ----




# ==== BAR CHARTS ====
# Bar chart(s) for BBCE policy variables; probably better to visualize then just displaying a table
# bc more work for consumer/user to do
# ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 9),
#        mapping = aes(x = factor(bbce_inclmt))) +
#   geom_bar(mapping = aes(fill = bbce_hh), color = "black") +
#   scale_y_continuous(limits = c(0, 14)) +
#   scale_fill_brewer(palette = "Oranges") +
#   facet_wrap(facets = .~bbce_sen) +
#   theme_bw() +
#   labs(title = "Federal Poverty Gross Income limit for SNAP qualification, household eligibility policy, and senior and disability household limits under BBCE",
#        subtitle = "Plot ribbons indicate ...",
#        x = "State-specified Federal Poverty Limit cut-off",
#        y = "Count")
# ^ This graph is too complex to read -> Make a new one are two separate graphs
...
ggplot(data = snap %>% filter(year(yearmonth) == 2016 & month(yearmonth) == 12),
  ...
  labs(x = "Asset Dollar Amount in Thousands",
       y = "Frequency",
       title = "SNAP policies by Asset Test and Asset Amount Limit under BBCE, Dec 2016")
# ^ Keep bar chart 3 ----
...
# Quick line graph of National SNAP outreach
qplot(x = yearmonth, 
      y = total.outreach, 
      data = outreach.total %>% filter(!is.na(total.outreach)), 
      geom = "line") +
  theme_bw()
...
plot_ly(data = outreach.total %>% filter(!is.na(total.outreach)),
  ...
  layout(title = "National SNAP Outreach, Fiscal Year 1996-2016",
         xaxis = list(title = "Year"),
         yaxis = list(title = "USD in Thousands"))
# Keep Line Graph 1 ^ ====


# plotly line graph example of outreach data by a specific State
# USE "state.ym.out" variable
plot_ly(data = state.ym.out %>% filter(state_pc == "DC"),
  ...
  layout(title = "SNAP Outreach over time, Fiscal Year 1996-2016",
         xaxis = list(title = "Year"),
         yaxis = list(title = "USD in Thousands"))
# Keep Line Graph 2 ====
...
# Create a plotly boxplot for outreach data (and possibly a layered jitter plot if possible) and add a slider to select fiscal year
# Otherwise, create boxplot and overlay point plot with user option to select fiscal year
...
state.ym.out <- snap %>%
  ...
  arrange(yearmonth) %>%
  filter(year(yearmonth) != 2016)
# ^ Important Data Frame / Global Variable ----
...
# Overlayed jitter plot on boxplot using ggplot2
# Filter to a specific fiscal year for an example
ggplot(data = state.ym.out %>% filter(year(yearmonth) == 2015), 
...
# KEEP boxplot overlaid with a jitter plot ^ ----
...
### Measures of Central Tendency
  - Calculate Median Values for outreach
    - Median is more reliable than Mean in this case bc outlying values
```{r}
# Calculate Central Tendency of SNAP outreach for each State from 1996-2016 fiscal years
snap %>%
  filter(month(yearmonth) == 10 | month(yearmonth) == 9 & year(yearmonth) == 1996) %>%
  group_by(state_pc) %>%
  summarize(avg_outreach = round(mean(outreach, na.rm = TRUE), 2), 
            median_outreach = round(median(outreach, na.rm = TRUE), 2), 
            missing_data = sum(is.na(outreach)), 
            percent_missing = sum(is.na(outreach))/n()*100) %>%
  ungroup()
# ^ Oct, Nov, Dec 2016 outreach data missing for all States (51 * 3 = 153)


# Calculate central tendency for ALL States in each fiscal year
snap %>%
  filter(month(yearmonth) == 10 | month(yearmonth) == 9 & year(yearmonth) == 1996) %>%
  ...
  summarize(avg_outreach = round(mean(outreach, na.rm = TRUE), 2), 
            ...
            percent_missing = sum(is.na(outreach))/n()*100) %>%
  ungroup()
# Should calculate directly bc issues w/ 1996 and 2016 fiscal years

# States w/ smallest median SNAP outreach FY 1996-2016 (10/2015 = FY 2016)
snap %>%
  filter((month(yearmonth) == 10 | (month(yearmonth) == 9 & year(yearmonth) == 1996))) %>%
  group_by(state_pc) %>%
  summarize(median_outreach = median(outreach, na.rm = TRUE)) %>%
...
### UI
- Create a Markdown chunk for dashboard Sidebar and Body?
  - dashboardBody will get lengthy and difficult to read
```{r sidebar}
...
- You should probably include definitions of policy variables that you use for the dashboard
  - Include in "About the Data Section"
```{r body}
body <- dashboardBody(
  ...
  tabItems(
    ...
    # ---- About the Data ----
    # Possibly use this section for Github README?
    tabItem(tabName = "about", 
            ...
            # ^ Edit or delete this section entirely ----
...
- Comment out global variables after saving to environment in case you accidentally run this code block
  - Don't want to overwrite data stored in R objects
```{r global variables}
# Get the outreach data for each State in each fiscal year and remove 2016 rows
# state.ym.out <- snap %>%
#   filter((month(yearmonth) == 10 | (month(yearmonth) == 9 & year(yearmonth) == 1996))) %>%
...
# Check where missing "transben" data occurs
#snap %>% filter(is.na(transben)) %>% select(yearmonth)
# Data is missing Nov-Dec 2016; We have transben data for FY 2017 start but no outreach data
...
server <- function(input, output) {
  ...
  # This graph does not render well at a small size (limited space in dashboard)
